...
PathInput.grid(row=0, column=4, sticky="ew")
SeparatorRows = [1, 3, 5, 7]
for row in SeparatorRows:
    ttk.Separator(Window, orient="horizontal").grid(row=row, column=0, columnspan=5, sticky="ew")
...
Console.grid(row=4, column=0, columnspan=5, sticky="ew")
...
TextBox.grid(row=8, column=1, columnspan=4, sticky="news")
if len(sys.argv) > 1:
    loadfile(sys.argv[1])
Window.mainloop()